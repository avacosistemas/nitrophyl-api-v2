...
public class InformeBuilder {
	...
	public ResponseEntity<JSONResponse> generarReporte(Lote lote)
...
//		cell.addElement(firmagraciela);
	...
	private Element addEnsayo(Ensayo ensayo) throws DocumentException {
		...
		for (EnsayoResultado resultado : ensayo.getResultados()) {
			...
			agregarPrueba(tableResultados, cell, nombre, minimo, maximo, resultado.getRedondeo(), norma, pos == ultimo);
			pos++;
		}
...
//		agregarPrueba(tableResultados, cell, "Tg δ", "0.04", "", "0.03", "NOR3", true);
		...
		cell.setPhrase(new Phrase(
				"Observaciones de remoetría de Observaciones de remoetría Observaciones de de remoetría Observaciones de de remoetría de de de Observaciones de remoetría Observaciones de remoetría Observaciones de remoetría ",
				fontText));
	...
	private void agregarPrueba(PdfPTable table, PdfPCell cell, String prueba, Double min, Double max, Double valor,
		...
		table.addCell(cell);
		cell.setHorizontalAlignment(Element.ALIGN_RIGHT);
		cell.setPhrase(new Phrase(min.toString(), fontText));
		table.addCell(cell);
		cell.setPhrase(new Phrase(max.toString(), fontText));
		table.addCell(cell);
		cell.setPhrase(new Phrase(valor.toString(), fontText));
		table.addCell(cell);
		cell.setHorizontalAlignment(Element.ALIGN_CENTER);
		cell.setPhrase(new Phrase(norma, fontText));
		table.addCell(cell);
