...
public class ClienteServiceImpl extends NJBaseService<Long, Cliente, ClienteRepository> implements ClienteService {
	...
	/*
	 * @Override public Cliente registrarClientePersona(Cliente cliente) throws
	 * ErrorValidationException, BusinessException { validarClienteNoVacio(cliente);
	 * validarAltaModificacionCliente(cliente);
	 * validaContactoCliente(cliente.getContacto()); cliente =
	 * registrarCliente(cliente); return cliente; }
	 * 
	 * //Valida que los requerimientos de contraseña sean validos private void
	 * validarContraseña(Cliente cliente) throws BusinessException { if
	 * (cliente.getPassword()==null) throw new
	 * BusinessException("Debe ingresar una contraseña."); }
	 * 
	 * //Valida que la composición del cliente este completa, que no sea null y que
	 * la //cuenta bancaria, el contacto y el ingreso tampoco sea null. private void
	 * validarClienteNoVacio(Cliente cliente) throws BusinessException { if (cliente
	 * == null) { throw new BusinessException("Cliente vacío."); } else if
	 * (cliente.getContacto() == null) { throw new
	 * BusinessException("Contacto vacío."); } }
	 * 
	 ...
	 * //cambiarse el password. Genera un password aleatorio y notifica al cliente
	 * la //bienvenida por mail. private Cliente registrarCliente(Cliente cliente) {
	 * // Por default el cliente se da de desbloqueado. cliente.setBloqueado(false);
	 * 
	 ...
	 * password. cliente.setRequiereCambioPassword(true);
	 * 
	 ...
	 * return cliente; }
	 * 
	 * @Override public void updatePassword(Cliente cliente, String password, String
	 * newPassword) { if (!passwordEncoder.matches(password, cliente.getPassword()))
	 * { throw new NuclearJSecurityException("Contraseña actual inválida."); } else
	 * if (password.equals(newPassword)) { throw new
	 * NuclearJSecurityException("La nueva contraseña es igual a la actual."); }
	 ...
	 * 
	 * public void resetPassword(Long idCliente) { Cliente cliente =
	 * repository.findOne(idCliente); // La cantidad de intentos de login es cero.
	 * cliente.setIntentosFallidosLogin(INICIO_REINTENTOS_LOGIN); // Se requerira
	 * cambio de password. cliente.setRequiereCambioPassword(true); // Genero una
	 * password temporal String tmppass = generarPasswordAleatorio();
	 * cliente.setPassword(passwordEncoder.encode(tmppass));
	 * getRepository().saveAndFlush(cliente);
	 ...
	 * registrado. public void validarAltaModificacionCliente(Cliente cliente)
	 * throws ErrorValidationException {
	 * 
	 * Map<String, String> errores = new HashMap<>();
	 * 
	 * // Validacion username if (StringUtils.isBlank(cliente.getUsername())) {
	 * errores.put("username", "El campo username es requerido."); } else if
	 * (cliente.getUsername().length() < 5) { errores.put("username",
	 * "El campo username debe tener al menos 5 caracteres."); } else if
	 * (!Validations.isUsernameValido(cliente.getUsername())) {
	 ...
	 * ); } else {
	 * 
	 * Cliente cliByUsername =
	 * getRepository().findByUsernameEqualsIgnoreCase(cliente.getUsername());
	 * 
	 * if(cliByUsername!=null && cliente.getId()==null || cliByUsername!=null &&
	 * cliente.getId()!=null && !cliente.getId().equals(cliByUsername.getId())) {
	 * 
	 * errores.put("username",
	 * "El username no esta disponible. Intente otro diferente."); }
	 * 
	 * cliByUsername = null;
	 * 
	 * }
	 * 
	 * // Validacion email if
	 * (!EmailValidator.getInstance().isValid(cliente.getEmail())) {
	 * errores.put("email", "El campo Email no tiene un formato válido."); } else {
	 * 
	 * Cliente cliByEmail =
	 * getRepository().findByEmailEqualsIgnoreCase(cliente.getEmail());
	 * 
	 * if(cliByEmail!=null && cliente.getId()==null || cliByEmail!=null &&
	 * cliente.getId()!=null && !cliente.getId().equals(cliByEmail.getId())) {
	 * 
	 ...
	 * }
	 * 
	 * // Validacion identificacion validaIdentificacion(cliente.getId(),
	 * cliente.getIdentificacion());
	 * 
	 * if (StringUtils.isBlank(cliente.getNombre()) ||
	 * StringUtils.isBlank(cliente.getApellido())) { errores.put("nombreApellido",
	 * "El campo Nombre y Apellido es requerido."); } else if
	 * (cliente.getNombre().length() > 30) { errores.put("nombre",
	 * "El campo Nombre no debe superar los 30 caracteres"); } else if
	 * (cliente.getApellido().length() > 30) { errores.put("apellido",
	 * "El campo Apellido no debe superar los 30 caracteres"); }
	 * 
	 * if (!errores.isEmpty()) {
	 * logger.error("Se encontraron los siguientes errores");
	 * errores.values().forEach((x->logger.error(x))); throw new
	 * ErrorValidationException("Se encontraron los siguientes errores", errores); }
	 * 
	 * }
	 * 
	 * @Override public void validaContactoCliente(Contacto contacto) throws
	 ...
	 * "Debe ingresar al menos un teléfono fijo o celular"); } else {
	 * 
	 * if (StringUtils.isBlank(contacto.getTelefonoMovil()) &&
	 ...
	 * }
	 * 
	 * public void validaIdentificacion(Long idCliente, Identificacion
	 * identificacion) throws ErrorValidationException {
	 * 
	 * Map<String, String> errores = new HashMap<>();
	 * 
	 * String numeroIdentificacion = identificacion.getNumero(); TipoIdentificacion
	 * tipoIdentificacion = identificacion.getTipo();
	 * 
	 * switch (tipoIdentificacion) { case DNI:
	 * 
	 * if (StringUtils.isBlank(numeroIdentificacion)) {
	 * errores.put("numeroIdentificacion", "El campo DNI es requerido."); } else if
	 * (!Validations.isDNIValido(numeroIdentificacion)) {
	 * errores.put("numeroIdentificacion",
	 * "El campo DNI Debe contener 7 u 8 caracteres numéricos"); } break; case CUIT:
	 * if (StringUtils.isBlank(numeroIdentificacion)) {
	 * errores.put("numeroIdentificacion", "El campo CUIT es requerido."); } else if
	 * (numeroIdentificacion.length() != 11) { errores.put("numeroIdentificacion",
	 * "El campo CUIT Debe tener exactamente 11 dígitos."); } else if
	 * (!Validations.isCuitValido(numeroIdentificacion)) {
	 * errores.put("numeroIdentificacion",
	 * "El campo CUIT debe comenzar con 20, 23, 24, 27, 30, 33 o 34."); } else if
	 * (!Validations.isDigitoVerificadorCuitCuilValido(numeroIdentificacion)) {
	 * errores.put("numeroIdentificacion",
	 * "El campo CUIT tiene el dígito verificador incorrecto."); } break; case CUIL:
	 * if (StringUtils.isBlank(numeroIdentificacion)) {
	 * errores.put("numeroIdentificacion", "El campo CUIL es requerido."); } else if
	 * (numeroIdentificacion.length() != 11) { errores.put("numeroIdentificacion",
	 * "El campo CUIL debe tener exactamente 11 dígitos."); } else if
	 * (!Validations.isCuilValido(numeroIdentificacion)) {
	 * errores.put("numeroIdentificacion",
	 * "El campo CUIL debe comenzar con 20, 23, 24 o 27."); } else if
	 * (!Validations.isDigitoVerificadorCuitCuilValido(numeroIdentificacion)) {
	 * errores.put("numeroIdentificacion",
	 * "El campo CUIL tiene el dígito verificador incorrecto."); }
	 * 
	 * break; default: break; }
	 * 
	 * Cliente cli =
	 * getRepository().findByIdentificacionNumeroLikeIgnoreCase(numeroIdentificacion
	 * );
	 * 
	 * if(cli != null && idCliente == null || cli != null &&
	 * !idCliente.equals(cli.getId())) {
	 * 
	 ...
	 * identificacionPorNumeroEncontrado.getNumero().substring(2,
	 * 9).equals(numeroIdentificacion)) { errores.put("numeroIdentificacion",
	 * tipoIdentificacion + " ya se encuentra registrado"); }
	 ...
	 * }
	 * 
	 * private String generarPasswordAleatorio() { String generateKey =
	 * KeyGenerators.string().generateKey(); return generateKey; }
	 * 
	 * @Override public UserDetails loadUserByUsername(String username) throws
	 * UsernameNotFoundException { return
	 * getRepository().findByUsernameEquals(username); }
	 * 
	 * @Override public Cliente getClientePorUsername(String username) { return
	 * getRepository().findByUsernameEquals(username); }
	 * 
	 * @Override public Cliente getClientePorMail(String username) { return
	 * getRepository().findByEmailEqualsIgnoreCase(username); }
	 * 
	 * @Override public Cliente getClienteCompleto(Long id) { return
	 * getRepository().getClienteCompleto(id); }
	 * 
	 * @Override public List<Cliente> listClientesListado() { return
	 * getRepository().listClientesListado(); }
	 * 
	 * public void setPasswordEncoder(PasswordEncoder passwordEncoder) {
	 * this.passwordEncoder = passwordEncoder; }
	 * 
	 * @Resource(name = "clienteRepository") void
	 * setClienteRepository(ClienteRepository clienteRepository) { this.repository =
	 * clienteRepository; }
	 * 
	 * @Resource(name = "notificacionService") public void
	 * setNotificacionService(NotificacionService notificacionService) {
